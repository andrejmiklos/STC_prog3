 ...
 * map1.set makes keys and gives them exact values
 */
export function someInfo() {
    JBooks.forEach( element => {
        map1.set(element.id, [element.id, element.name, element.author, element.genre])
    });
}
 ...
 * JBooks.forEach checks every element in JBooks
 * map2.set makes keys and gives them values based on values in that element
 */
